...
compute_forces(void)
{
	...
	*/
	for (b=0; b<bodyCt; ++b) {
      ...
      //printf("Drawing[%d,%d]: %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", b, c, X(c), X(b), Y(c), Y(b), R(b), R(c), M(b), M(c), XF(b), YF(b), XF(c), YF(c));
			double dx = X(c) - X(b); // <== communication
			double dy = Y(c) - Y(b); // <== communication
			double angle = atan2(dy, dx);
			double dsqr = dx*dx + dy*dy;
			double mindist = R(b) + R(c); // <== communication
			double mindsqr = mindist*mindist;
			double forced = ((dsqr < mindsqr) ? mindsqr : dsqr);
			double force = M(b) * M(c) * GRAVITY / forced; // <== communication
...
main(int argc, char **argv)
{
	...
	for (b=0; b<bodyCt; ++b) {
		X(b) = (rand() % xdim);
		Y(b) = (rand() % ydim);
    R(b) = 1 + ((b*b + 1.0) * sqrt(1.0 * ((xdim * xdim) + (ydim * ydim)))) / (25.0 * (bodyCt*bodyCt + 1.0));
		M(b) = R(b) * R(b) * R(b);
